...
${member.toString()} üyesinin ismi "${name} | ${age}" ismine değiştirildi üyenin cinsiyetini belirtmek için aşağıdaki butonları kullanın.`)
    ...
    collector.on("collect", async (button) => {
      ...
      if(button.customId === "erkek") {
        ...
        if (!names) {
            bgerkekemb.setDescription(`${member} üyesine <@&${config.registration.oneman}> rolü verildi.`) 
        } else {
            bgerkekemb.setDescription(`${member} üyesine <@&${config.registration.oneman}> rolü verildi.\n\n<:no_toasty:1039603137343930368> Bu üye daha önceden de bu isimler ile de kayıt olmuş: \n${names.map((data) => `${data}`).join("\n")}
...
msg.edit({
  ...
  if(button.customId === "kadin") {
    ...
    if (!names) {
        bgkadinemb.setDescription(`${member} üyesine <@&${config.registration.onewoman}> rolü verildi.`) 
    } else {
        bgkadinemb.setDescription(`${member} üyesine <@&${config.registration.onewoman}> rolü verildi.\n\n<:no_toasty:1039603137343930368> Bu üye daha önceden de bu isimler ile de kayıt olmuş: \n${names.map((data) => `${data}`).join("\n")}
