...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        if (!names) return message.reply({ embeds: [embed.setDescription(`Bu üyenin geçmiş isimleri bulunamadı.`)] }).catch((err) => console.log(err), client.ytick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
        message.reply({ embeds: [embed.setTitle("Bu üyenin geçmiş isimleri:").setDescription(names.map((data, n) => `**${n + 1}.** ${data}`).join("\n"))] }).catch((err) => console.log(err), client.ytick(message)).then((e) => setTimeout(() => { e.delete(); }, 10000));
    }
}
